...
def config_common_part(port, ssl_bind_string, vhosts):
    ...
    if not SKIP_FORWARDED_PROTO:
        if ssl:
            frontend_section.append("http-request set-header X-Forwarded-Proto https")
        else:
            frontend_section.append("http-request set-header X-Forwarded-Proto http")
...
def config_default_frontend_by_port(port, ssl_bind_string):
    ...
    if ssl_bind_string:
        ...
        # add x-forwarded-porto header if not skipped
        if not SKIP_FORWARDED_PROTO:
            frontend.append("http-request set-header X-Forwarded-Proto https")
    else:
        frontend = [("bind :%s %s" % (port, EXTRA_BIND_SETTINGS.get(port, ""))).strip()]
        if not SKIP_FORWARDED_PROTO:
            frontend.append("http-request set-header X-Forwarded-Proto http")
